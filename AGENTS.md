...
- Keep HindsightKit a setup wrapper around pinned official Hindsight components. Optional connectors use a small explicit registry and isolated adapters; do not add a memory database, learning pipeline, or forked engine.
...
- Live tests use disposable repository banks and remove their own shared documents. `tests/live_memory.py` uses Copilot allowance; preserve unrelated memory and services.
