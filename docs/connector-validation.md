...
## September 19, 2026: partial sync feedback and retries

All 148 mail tests and 39 connector tests passed with `PYTHONPATH` set to the checkout's `src` directory. `node --check src/hindsightkit/web/connectors.js` also passed. Regression checks covered partial completion without a global error, source and discovery failures, and retries of failed publication operations. After a source recovered, the next sync processed only the failed threads, preserved the unchanged successful document, cleared the failure list, and kept the schedule enabled.

A local browser fixture verified that failure counts and details stay hidden during a run, appear on completion even when the list has not changed, and disappear when all updates succeed. **Sync now** submitted the existing sync action and hid the previous summary while running. Partial completion showed the correct message for enabled and paused schedules; a connection failure still displayed an error. These checks used synthetic data without mailbox reads or model calls.
