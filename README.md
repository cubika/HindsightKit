...
The v0.1.6 preview supports Windows x64 with Windows PowerShell 5.1 or PowerShell 7. Install Git and have a Copilot account available for sign-in. Use the commands from the published version you select; `-ClientOnly` requires v0.1.2 or later.
...
The installer downloads the application and pinned Python/npm dependencies as separate, hash-checked archives. Upgrades reuse cached dependencies when their contents have not changed. Python and, when needed, Node.js still download; a new local server also needs the embedding model and precompiled PostgreSQL/pgvector distribution. No source checkout or C++ compiler is needed. Server setup checks a temporary memory through Copilot and removes it afterward. See [troubleshooting](docs/installation.md#dependency-installation-and-retries) for retries, and [release validation](docs/release-validation.md) for published versions and test results.
