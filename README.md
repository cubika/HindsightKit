...
The current preview supports **Windows x64** with Windows PowerShell 5.1 or PowerShell 7. Install Git and have a Copilot account available for sign-in.
...
The default command installs the local server and coding integrations together. It downloads Python, Node.js, dependencies, and a precompiled PostgreSQL/pgvector package. No source checkout or C++ compiler is needed. Setup checks a temporary memory through the real model provider and removes the test bank afterward; this uses Copilot allowance.

The v0.1.0 preview has a reported Python dependency download failure on some networks: `files.pythonhosted.org` returns TLS `HandshakeFailure`. See [installation troubleshooting](docs/installation.md#troubleshooting) before retrying. Successful release builds do not guarantee that every client network can reach the download hosts.
